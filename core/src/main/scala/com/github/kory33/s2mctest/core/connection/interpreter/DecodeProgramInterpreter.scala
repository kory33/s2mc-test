...
object DecodeProgramInterpreter {
  ...
  private def runInstructionM[M[_]: ReadBytes: HasRemainingByteCount: RaiseParseError]
    : DecodeScopedBytesInstruction ~> M = {
        ...
        (instruction: DecodeScopedBytesInstruction[A]) =>
          {
            ...
            def readChunkSafely(length: Int): M[Chunk[Byte]] =
              for {
                ...
                chunk <-
                  if (length <= remainingBytes) then ReadBytes[M].ofSize(length)
                  else
                    RaiseParseError[M].raise[ParseInterruption, Chunk[Byte]](
                      ParseInterruption.RanOutOfBytes
                    )
            ...
            instruction match {
              ...
              case ReadEntireScope =>
                HasRemainingByteCount[M].get.flatMap(readChunkSafely).widen
              case ps: PreciseScope[a2] =>
                interpretOnChunkAndGet[M, a2](ps.chunk, ps.program).widen
              case RaiseError(err) =>
                RaiseParseError[M].raise[ParseInterruption, A](ParseInterruption.Raised(err))
              case Giveup(reason) =>
                RaiseParseError[M].raise[ParseInterruption, A](ParseInterruption.Gaveup(reason))
            }
          }: M[A]
    )
  }

  private def compileToM[M[_]: ReadBytes: HasRemainingByteCount: RaiseParseError]
    : DecodeScopedBytes ~> M =
    DecodeScopedBytes.asFreeK.andThen {
      cats
        .free
        .Free
        .foldMap[DecodeScopedBytesInstruction, M](runInstructionM[M])(
          using HasRemainingByteCount[M].monad
        )
    }
  ...
  private def readBytesForChunkContext[F[_]: WithRemainingByteChunk: RaiseParseError]
    : ReadBytes[F] =
    new ReadBytes[F] {
      override def ofSize(n: Int): F[Chunk[Byte]] =
        ...
        for {
          ...
          split <-
            if (n <= current.size) then Monad[F].pure(current.splitAt(n))
            else Raise.raiseF[F](ParseInterruption.RanOutOfBytes)
   ...
   *
   * If the program does not crash (that is, correctly raises error when encountering bad
   * input), the effect is guaranteed to read exactly [[size]] bytes, and otherwise the entire
   * effect would fail.
   */
  def interpretWithSize[F[_]: Monad: ReadBytes, A](
    size: Int,
    program: DecodeScopedBytes[A]
  ): F[ParseResult[A]] =
    type Execution = EitherT[StateT[F, Int, _], ParseInterruption, _]

    compileToM[Execution].apply(program).value.run(size).flatMap {
      case (remainingBytes, result) =>
  ...
  def interpretOnChunk[A](chunk: Chunk[Byte], program: DecodeScopedBytes[A]): ParseResult[A] =
    ...
    else flattenedResult

  /**
   * Interpret the decode program purely on the provided chunk and get the result, but throw in
   * the context of [[F]] if obtained [[ParseResult]] was a failure.
   */
  private def interpretOnChunkAndGet[F[_]: Applicative: RaiseParseError, A](
    chunk: Chunk[Byte],
    program: DecodeScopedBytes[A]
  ): F[A] =
