...
trait PacketTransport[F[_]] {
   ...
   *  - should return the entire chunk that corresponds to a packet,
   *    so that the next read will produce valid PacketID-PacketData sequence
   */
   ...
   *
   * This action must acquire mutex guard of the underlying write buffer to avoid multiple writes occuring at once.
   */
