...
object MonadValueExt {
  ...
  extension [F[_], A](fa: F[A])
     ...
     *
     * This implementation uses append on each evaluation result, so avoid data structures with
     * non-constant append performance, e.g. `List`.
     */
    def repeatWhileM[G[_]](
      condition: A => Boolean
    )(using F: cats.Monad[F], G: cats.Alternative[G]): F[G[A]] =
